...
const Panel: React.FC<Props> = ({texts, title, button}) => {
  return <section
    className={`p-6 panel relative backdrop-blur-lg backdrop-filter mb-10 flex flex-col ${rajdhani.className} fade-in mx-4 el max-w-[48rem] self-center`}>
    <h2 className="my-4 md:text-4xl sm:text-3xl text-4xl font-bold leading-8 text-center">{title}</h2>
    {
      // eslint-disable-next-line react/no-unescaped-entities
      texts.map((item, key) => (
        <p key={key} className="my-4 panel__description" dangerouslySetInnerHTML={{__html: item}}/>)
      )
    }
    <div className="justify-center flex my-4">
      <a href="" className="home__btn text-2xl">{button.label}</a>
    </div>
    <div
